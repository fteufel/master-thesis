...
#AWD-LSTM model unchanged until decoding step (standard: multiply with embeddings to get probs)
# this step was changed as described in hyperbolic_embeddings.md
...
class PoincareReparametrize(nn.Module):
    '''Module to reparametrize vectors to fit the Poincare ball, as described in
    https://www.aclweb.org/anthology/W18-1708.pdf
    '''
    ...
    def forward(self, x):
        ...
        v_bar_norm = torch.unsqueeze(torch.norm(v_bar, dim = -1), -1) #add dummy dim where output_dim was
        v = v_bar / v_bar_norm 
        p = torch.sigmoid(p_bar)
...
class ProteinAWDLSTMModel(ProteinAWDLSTMAbstractModel):
    ...
    def __init__(self, config: ProteinAWDLSTMConfig, is_LM = False):
        ...
        self.poincare_map = PoincareReparametrize(config.input_size)
    ...
    def forward(self, input_ids, input_mask = None, hidden_state = None):
        ...
        #map output to poincare ball
        output = self.poincare_map(output)
...
class ProteinAWDLSTMForLM(ProteinAWDLSTMAbstractModel):
    ...
    def __init__(self, config):
        ...
        self.decoder = nn.Linear(config.hidden_size, config.vocab_size, bias=False) #No bias
    ...
    def poincare_dist(self, u, v):
        '''Adapted from https://github.com/facebookresearch/poincare-embeddings/blob/5f465041619a63bf1de99c55d1298865bf70fd9e/hype/manifolds/poincare.py
        Don't see why manual implementation of backwards pass is necessary, it is all built from torch functions and autograd should take care of it 
    ...
    def forward(self, input_ids, input_mask=None, hidden_state = None, targets =None):
        ...
        # Poincare embedding step, sequence_output already comes mapped to poincare from encoder
        embedding_vectors = self.encoder.embedding_layer.weight

        #instead of inner products with embeddings, get distances to poincare mappings of embeddings
        ...
        for i in range(embeddings_hyp.shape[1]):
            ...
            distlist.append(self.poincare_dist(sequence_output, emb_vector))

        prediction_scores = torch.stack(distlist, -1) *-1 #make negative, high distance is a low score
