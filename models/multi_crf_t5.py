...
from transformers import T5Model, T5EncoderModel, T5Tokenizer, T5PreTrainedModel
...
class T5SequenceTaggingCRF(T5PreTrainedModel):
    ...
    def forward(self, input_ids = None, kingdom_ids = None, input_mask=None, targets =None, targets_bitmap=None, global_targets = None, inputs_embeds = None,
        ...
        outputs = self.encoder(input_ids, attention_mask = input_mask, inputs_embeds = inputs_embeds, use_cache=False) # Returns tuple. pos 0 is sequence output, rest optional.
        ...
        ## CRF
        if targets is not None:
            targets = targets.to(prediction_logits.device)
            input_mask = input_mask.to(prediction_logits.device)
            ...
            targets_bitmap = targets_bitmap.to(prediction_logits.device)
            input_mask = input_mask.to(prediction_logits.device)
    ...
    def parallelize(self, device_map=None):
        ...
        # get the highest index in device map
        # additional modules need to be on same device as last block
        device_of_last_layer=0
        last_layer=0
        for k in self.device_map.keys():
            if max(self.device_map[k]) >last_layer:
                device_of_last_layer = k
            
        self.outputs_to_emissions.to(device_of_last_layer)
        self.crf.to(device_of_last_layer)


    def deparallelize(self):
        self.encoder.deparallelize()
        self.encoder = self.encoder.to("cpu")
        self.outputs_to_emissions.to("cpu")
        self.crf.to("cpu")
        self.model_parallel = False
        self.device_map = None
