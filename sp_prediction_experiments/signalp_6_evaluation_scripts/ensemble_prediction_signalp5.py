...
from train_scripts.utils.signalp_dataset import SIGNALP5_VOCAB, SIGNALP_KINGDOM_DICT
...
def run_data_array(model: torch.nn.Module, input_ids:  np.ndarray, input_mask: np.ndarray, kingdom_ids: np.ndarray, batch_size = 100):
    ...
    while b_start < len(input_ids):
        ...
        mask = mask.to(device)
        k_ids = kingdom_ids[b_start:b_end]
        k_ids = torch.tensor(k_ids)
        k_ids = k_ids.to(device)


        with torch.no_grad():
            global_probs, pos_probs, pos_preds = model(data, kingdom_ids=k_ids, global_targets = None, input_mask = mask)
...
def run_data_ensemble(model: torch.nn.Module, 
    ...
    for path in tqdm(checkpoint_list):
        ...
        results = run_data_array(model_instance, input_ids, input_mask, kingdom_ids)
...
def main():
    ...
    #filter sequences that cannot be predicted
    bad_seqs = seqs.str.contains("U|X|Z")
    seqs = seqs.loc[~bad_seqs]
    df = df.loc[~bad_seqs]

    tokenized = seqs.apply(lambda x: [SIGNALP5_VOCAB[t] for t in x])
    tokenized =  tokenized.apply(lambda x: x + [0] * (70-len(x))) #pad 
    input_ids = np.vstack(tokenized)

    input_mask = (input_ids>0) *1
    kingdom_ids = np.array([SIGNALP_KINGDOM_DICT[args.kingdom]] * len(seqs))
