...
if __name__ == '__main__':
    ...
    df = pd.read_csv('sp/sp_uniprot_hits.tsv', sep='\t')
    ...
    df_false = pd.concat([df1, df2, df3, df4])

    len_before = len(df)
    df = df.merge(df_false['Entry name'].drop_duplicates(), on=['Entry name'], how='left', indicator=True)
    df = df[df['_merge'] == 'left_only']

    print(f'Removed TAT/LIPO hits. {len(df)} of {len_before} sequences remaining.')
    ...
    # No contradictions are removed, as the evidence for the SP is experimental.
    # SP beats TM.
    ...
    with open('sp.fasta', 'w') as f:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                tm_indices =row['tm_positions'] if not np.isnan(row['tm_positions']).any() else None

            label = generate_label_sequence(row['sp_end'], seq_len = len(row['Sequence']), tm_indices = tm_indices ,sp_symbol ='S' )
