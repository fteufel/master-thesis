...
if __name__ == '__main__':
    ...
    df = pd.read_csv('tat/tat_uniprot_prosite_matches.tsv', sep='\t')
    df2 = pd.read_csv('tat/tat_uniprot_non_prosite_matches.tsv', sep='\t')
    ...
    # Remove unknowns
    df_false = pd.read_csv('tat/tat_unknown.tsv', sep='\t')
    ...
    df_scanprosite = pd.read_csv('tat/tat_scanprosite_prosite_matches.tsv', sep='\t', names= ['id', 'match_start', 'match_end', 'profile', 'score', '5','6','7'])
    ...
    with open('tat.fasta', 'w') as f:

        for idx, row in df.iterrows():
            header = row['Entry'] + '|' + row['kingdom'] +  '|' + 'TAT'

            try:
                tm_indices =  df_topdb[row['Entry name']]
                ...
                #TODO not using Uniprot yet here
                tm_indices = None
            label = generate_label_sequence(row['sp_end'], seq_len = len(row['Sequence']), tm_indices = tm_indices ,sp_symbol ='T' )
