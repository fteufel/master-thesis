...
def generate_label_sequence(sp_len: int, tm_indices: List[Tuple[int,int]] = None, seq_len=70, sp_symbol = 'S'):
    ...
    '''
    labels = np.array(['O'] * seq_len)

    # Start with SP.
    labels[0:int(sp_len)] = sp_symbol

    if tm_indices is not None and len(tm_indices)>0:
        is_outside=True # flag to keep track where I am when multiple sp regions
        for (start, end) in tm_indices:
            
            labels[start-1:end] = 'M'
            
            if is_outside: 
                labels[end:] = 'I'
                is_outside = False #now am inside
            else:
                labels[end:] = 'O'
                is_outside = True #now am outside again
...
def string_to_structdf(string):
    '''pseudo-uniprot topdb string feature descriptor to struct df.
    Only returns features that are considered membrane.'''
    ...
    structdf =  pd.DataFrame.from_dict(outlist)
    structdf =  structdf[structdf['Localisation'].apply(lambda x: x in membrane_names)]
