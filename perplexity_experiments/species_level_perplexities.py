...
def run_model_on_data(model, dataset) -> float:
    ...
    for i, batch in enumerate(dl):
        ...
        total_loss += loss.item()*data.shape[1]
...
for org in tqdm(organisms.index):
    ...
    print(f'{org}, Perplexity {np.exp(loss)}, Loss {loss}')
...
df_out = pd.DataFrame([pd.Series(results_dict), pd.Series(len_dict)], index = ['perplexity', 'n_sequences']).T
