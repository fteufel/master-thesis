...
def validate(model: torch.nn.Module, valid_data: DataLoader) -> float:
    ...
    for i, batch in enumerate(valid_data):
        ...
        data = data.to(device)
        targets = targets.to(device)
        input_mask = input_mask.to(input_mask)
    ...
    all_probs = np.concatenate(all_global_probs)

    try:
        global_roc_curve = wandb.plots.ROC(all_global_targets, all_global_probs, [0,1,2,3,4,5,6,7])
    except: 
