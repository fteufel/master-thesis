'''
Train a CRF sequence tagging and global label prediction model on top of the TAPE pfam-bert model.
...
def tagged_seq_to_cs_multiclass(tagged_seqs: np.ndarray, sp_tokens = [0,4,5]):
    ...
    def get_last_sp_idx(x: np.ndarray) -> int:
        ...
        sp_idx = np.where(x == np.isin(sp_tokens, x))[0]
...
def main_training_loop(args: argparse.ArgumentParser):
    ...
    config = ProteinBertConfig.from_pretrained(args.resume)
    ...
    model = ProteinLMSequenceTaggingCRF.from_pretrained(args.resume, config = config)    
    #training logger
    ...
    train_data = PartitionThreeLineFastaDataset(args.data, partition_id = [0,1,2,3], add_special_tokens = True )
    val_data = PartitionThreeLineFastaDataset(args.data, partition_id = [4], add_special_tokens = True)
