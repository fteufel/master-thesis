...
def main_training_loop(args: argparse.ArgumentParser):
    ...
    model = ProteinAWDLSTMForLM.from_pretrained(args.resume, config = config)    
    #training logger
    ...
    logger.info(f'Data loaded. One epoch = {len(train_data)} sequences.')
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for i, batch in enumerate(train_loader):
            ...
            data, targets = batch
            loss, hidden = training_step(model, data, targets, optimizer, args, i)
